...
training = training %>%
  mutate(isWristshot = ifelse(shotType == "wrist", 1, 0)) %>%
  mutate(isSlapshot = ifelse(shotType == "slap", 1, 0)) %>%
  mutate(isTipin = ifelse(shotType == "tip-in", 1, 0)) %>%
  mutate(isSnapshot = ifelse(shotType == "snap", 1, 0)) %>%
  mutate(isWraparound = ifelse(shotType == "wrap-around", 1, 0)) %>%
  mutate(isBackhand = ifelse(shotType == "backhand", 1, 0)) %>%
  mutate(isPoke = ifelse(shotType == "poke", 1, 0)) %>%
  mutate(isDeflection = ifelse(shotType == "deflected", 1, 0)) %>%
  mutate(isBatted = ifelse(shotType == "bat", 1, 0)) %>%
  mutate(isCradle = ifelse(shotType == "cradle", 1, 0)) %>%
  mutate(isBetweenlegs = ifelse(shotType == "between-legs", 1, 0)) %>%
  mutate(isRushbin = ifelse(isRush == "True", 1, 0)) %>%
  mutate(isReboundbin = ifelse(isRebound == "True", 1, 0)) %>%
  mutate(createdReboundbin = ifelse(createdRebound == "True", 1, 0))
...
just_to_see = testing %>%
  select(goalcheck, predicted_prob, strengthState, distance_lagged, angle_lagged, timelastevent, isEmptyNetAgainst, shotzone, angle, distance, shotType)
...
write.csv(grouped_all, "grouped_all.csv")
